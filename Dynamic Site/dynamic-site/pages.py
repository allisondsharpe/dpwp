...
class ContentPage(Page): #ContentPage() class created to contain the getters and setters
    ...
    def new_print_out(self): #Polymorphism function - Will return all values that would otherwise be created within the template
        return self._head + self._body + '<div id="container">' + self._header_img + '<nav>' + '<li><a href="?nav=home" class="btn"> Home </a></li>' + '<li><a href="?nav=about" class="btn"> About </a></li>' + '<li><a href="?nav=careers" class="btn"> Careers </a></li>' + '<li><a href="?nav=faq" class="btn"> FAQ </a></li>' + '<li><a href="?nav=contact" class="btn"> Contact </a></li>' + '</nav>' + '<h1>' + self._header + '</h1>' + '<h2>' + self._the_body + '</h2>' +  '<p>' + self._footer  + '</p>' + '<p>' + str(self._current_year) + '</p>'  + '</div>' + self._close